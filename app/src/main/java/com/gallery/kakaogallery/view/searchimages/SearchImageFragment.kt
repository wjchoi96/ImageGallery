...
import com.gallery.kakaogallery.view.base.BaseFragmentUseHandler
...
class SearchImageFragment : BaseFragmentUseHandler<FragmentSearchImageBinding, SearchImageViewModel>() {
    ...
    private fun setListener(){
        vd.recyclerView.setOnTouchListener { v, event ->
            when(event?.action){
                MotionEvent.ACTION_UP -> hideKeyBoard(v)
            }
...
//        }
            ...
            ImageModel.Payload.PayloadType.Changed -> {
                ...
                when(payload.changedPayload){
                    ...
                    ImageModel.Payload.ChangedType.Select -> {
                        for(idx in payload.changedIdx){
                            imageSearchAdapter.notifyItemChanged(idx + 1, SearchImagesAdapter.ImagePayload.Select)
                        }
                    }
                }
            }
    ...
    private fun setProgressPaging(visible : Boolean){
        loading = visible
        if(visible)
            vd.listProgress.visibility = View.VISIBLE
        else
            vd.listProgress.visibility = View.GONE
    }
